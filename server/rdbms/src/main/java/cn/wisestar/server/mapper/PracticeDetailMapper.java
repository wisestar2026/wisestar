...
public interface PracticeDetailMapper extends BaseMapper<PracticeDetail> {
	...
	@Select("<script>"
			...
			+ "       x.knowledgePointId, x.knowledgePointName, "
			+ "       x.subjectId, x.subjectName, x.chapterId, x.chapterName, x.sectionId, x.sectionName, x.grade "
			+ "FROM ( "
			...
			+ "         kpq.knowledge_point_id AS knowledgePointId, kp.name AS knowledgePointName, "
			+ "         ch.subject_id AS subjectId, sub.name AS subjectName, "
			+ "         ch.id AS chapterId, ch.name AS chapterName, "
			+ "         s.id AS sectionId, s.name AS sectionName, "
			+ "         COALESCE(s.grade, ch.grade) AS grade, "
			...
			+ "  LEFT JOIN t_knowledge_point_question kpq ON d.question_id = kpq.question_id AND kpq.is_deleted = 0 "
			+ "  LEFT JOIN t_knowledge_point kp ON kpq.knowledge_point_id = kp.id AND kp.is_deleted = 0 "
			+ "  LEFT JOIN t_section s ON kp.section_id = s.id AND s.is_deleted = 0 "
			+ "  LEFT JOIN t_chapter ch ON s.chapter_id = ch.id AND ch.is_deleted = 0 "
			+ "  LEFT JOIN t_subject sub ON ch.subject_id = sub.id AND sub.is_deleted = 0 " // t_repo 为遗留表，无 is_deleted 列，不做逻辑删除过滤
			...
			+ "    AND ch.subject_id = #{query.subjectId} "
			+ "  </if> "
			+ "  <if test=\"query.chapterId != null and query.chapterId != ''\"> "
			+ "    AND s.chapter_id = #{query.chapterId} "
			+ "  </if> "
			+ "  <if test=\"query.sectionId != null and query.sectionId != ''\"> "
			+ "    AND s.id = #{query.sectionId} "
			+ "  </if> "
			+ "  <if test=\"query.knowledgePointId != null and query.knowledgePointId != ''\"> "
			+ "    AND kp.id = #{query.knowledgePointId} "
			+ "  </if> "
			+ "  <if test=\"query.grade != null and query.grade != ''\"> "
			+ "    AND COALESCE(s.grade, ch.grade) = #{query.grade} "
			+ "  </if> "
			+ "  <if test=\"query.wrongReason != null and query.wrongReason != ''\"> "
			+ "    AND d.wrong_reason = #{query.wrongReason} "
			+ "  </if> "
