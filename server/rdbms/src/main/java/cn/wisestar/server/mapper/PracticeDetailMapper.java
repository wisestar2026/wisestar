...
public interface PracticeDetailMapper extends BaseMapper<PracticeDetail> {
	...
	@Select("<script>"
			...
			+ "       x.userId, x.userName, x.wrongCount, x.lastWrongTime, x.lastAnswer, x.lastScore "
			+ "FROM ( "
			...
			+ "         d.user_answer AS lastAnswer, d.score AS lastScore, "
			...
			+ "  JOIN t_user u ON r.user_id = u.id AND u.is_deleted = 0 "
